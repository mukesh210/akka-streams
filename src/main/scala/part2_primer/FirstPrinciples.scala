...
object FirstPrinciples extends App {
  ...
  implicit val materializer = ActorMaterializer()  // enables running of Akka streams component
...
//  source.via(flow).to(sink).run()
  ...
  val nameSource = Source(List("Alice", "Bob", "Charlie", "David", "Martin", "AkkaStreams"))
  ...
  // nameSource.via(longNameFlow).via(limitFlow).runForeach(println)
